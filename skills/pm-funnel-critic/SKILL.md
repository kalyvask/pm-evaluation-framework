...
Don't use this when:
- The strategy isn't framed yet — route to `pm-decision-coach` or `${CLAUDE_PLUGIN_ROOT}/decision-making/problem-framing.md` first
...
Before critiquing the funnel, name which model applies: **Enterprise (sales-led)** or **PLG (product-led)**. Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/user-vs-chooser.md` if it's unclear from context. The model determines which benchmarks to apply and which drop-off drivers to look for.
...
For each underperforming stage, name the driver from the catalogue in `${CLAUDE_PLUGIN_ROOT}/decision-making/conversion.md`. The common ones:
...
- **Cites the repo.** Point at `${CLAUDE_PLUGIN_ROOT}/decision-making/activation.md` § [gate], `${CLAUDE_PLUGIN_ROOT}/decision-making/conversion.md` § [driver], `${CLAUDE_PLUGIN_ROOT}/decision-making/user-vs-chooser.md` § [tier] so the reviewer can deepen the read.
