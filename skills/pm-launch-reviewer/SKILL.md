...
Reviews a launch plan and returns a gate-by-gate verdict against `${CLAUDE_PLUGIN_ROOT}/templates/launch-criteria.md`, plus a failure-mode pass against `${CLAUDE_PLUGIN_ROOT}/cross-functional/failure-management.md`. The job is to catch the things that look fine on paper but cause incidents in week one — unowned gates, vague readiness criteria, missing rollback plans, and over-confidence about user behavior.
...
- The launch already shipped and the user wants a postmortem — use `${CLAUDE_PLUGIN_ROOT}/templates/blameless-postmortem.md` directly
...
2. **Walk every gate from `${CLAUDE_PLUGIN_ROOT}/templates/launch-criteria.md`.** Technical, product, GTM, support, legal/compliance, executive sign-off. For each gate, check three things:
...
4. **Run the failure-management lens.** From `${CLAUDE_PLUGIN_ROOT}/cross-functional/failure-management.md`: what's the most likely failure in week one? What's the rollback path? Who has the authority to pull the plug, and at what threshold? If "we'll figure it out" is the answer to any of these, the plan isn't ready.

5. **Apply the bundle/journey check.** Reference `${CLAUDE_PLUGIN_ROOT}/rubrics/pm-evaluation-rubric.md` criterion 3. Does the launch plan account for procurement, admin onboarding, support staffing, and any downstream team that will field questions? Most launch failures are *adjacent* failures — the feature works, but the support team didn't know it was launching.
