...
Don't use this when:
- The problem isn't framed yet — run `pm-decision-coach` or `${CLAUDE_PLUGIN_ROOT}/decision-making/problem-framing.md` first; design critique on a wrong problem is wasted
...
Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/behavioral-design.md`. For each moment, ask:
...
Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/ai-integration.md`. If the design includes AI features, check:
...
The checklist above is for AI that produces an output the user then applies. If the design has the AI taking multi-step actions on the user's behalf, or writing to shared state (sending, updating, committing, publishing), reference `${CLAUDE_PLUGIN_ROOT}/decision-making/agentic-product-design.md` and check these as well — note that the last bullet above inverts, because a user accountable for an agent's work needs to see what it did:
...
- **Distinguishes design holes from strategy holes.** If the issue is "wrong target user," route to `pm-red-team` or `${CLAUDE_PLUGIN_ROOT}/decision-making/problem-framing.md`. Don't smuggle a strategy critique into a design critique.
- **Cites the repo.** When naming a principle, point at `${CLAUDE_PLUGIN_ROOT}/decision-making/behavioral-design.md` § [principle] so the reviewer can deepen the read. Same for AI surface critiques and `${CLAUDE_PLUGIN_ROOT}/decision-making/ai-integration.md`.
