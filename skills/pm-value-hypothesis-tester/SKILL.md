...
Don't use this when:
- The product already has clear PMF and the question is **how to grow** — use `${CLAUDE_PLUGIN_ROOT}/frameworks/04-post-pmf-growth.md`
- The user wants to **measure existing PMF** — use `${CLAUDE_PLUGIN_ROOT}/frameworks/03-pre-pmf-validation.md` and `pm-metrics-critic`
...
1. **Force the one-line hypothesis.** Push the user to write: *"For [specific desperate who], we will build [specific what] enabled by [specific insight], monetized via [specific how]."* If any blank is generic, stop and re-write before evaluating anything else. A vague hypothesis cannot be falsified, which means it's not a hypothesis. Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` § "What a value hypothesis is."

2. **Stress-test the insight (the what).** What changed — technologically, behaviorally, or in regulation — that makes this newly possible? If the answer is *"nothing changed, we just think the existing solution is bad,"* that's a sustaining innovation, not a non-consensus bet. Apply Rachleff's right/non-consensus filter. Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` § "The 'what'." Cross-reference `${CLAUDE_PLUGIN_ROOT}/decision-making/problem-framing.md` § "Problem vs. opportunity."
...
4. **Check Geoffrey Moore positioning.** Where on the adoption curve is the target? If the team is pitching to the early/late majority before crossing the chasm with early-adopter references, that's a chasm jump and it usually fails. Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` § "Geoffrey Moore's adoption curve."
...
6. **Stress-test the how.** Ask:
   ...
   - Could this be made *disruptive* in the Christensen sense (simpler, cheaper, more convenient — uneconomic for incumbents to copy)? Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` § "The 'how'."
...
- Treating one earlyvangelist as PMF. One desperate user is a starting point; the bowling-pin needs ten before the segment is meaningfully validated. Reference `${CLAUDE_PLUGIN_ROOT}/frameworks/03-pre-pmf-validation.md` § "10 happy users."
