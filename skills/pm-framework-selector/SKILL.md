...
2. **Map the decision to a lifecycle phase**, using the table below. Frameworks live in `${CLAUDE_PLUGIN_ROOT}/frameworks/01-strategy-and-discovery.md` through `${CLAUDE_PLUGIN_ROOT}/frameworks/05-continuous-prioritization.md`.
...
| Whether the underlying value-hypothesis bet (what / who / how) holds together | Pre-PMF Validation | Rachleff value hypothesis, Christensen disruption, bowling-pin / chasm — see `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` |
...
| How to actually run a customer-discovery interview without collecting false positives | Strategy & Discovery / Pre-PMF | Mom Test rules — see `${CLAUDE_PLUGIN_ROOT}/decision-making/customer-interviews.md` |
...
For decisions that don't fit a phase cleanly (problem framing, research methods, cross-functional disputes), look in `${CLAUDE_PLUGIN_ROOT}/decision-making/` and `${CLAUDE_PLUGIN_ROOT}/cross-functional/`:


| Question | File |
|---|---|
| Am I solving the right problem? | `${CLAUDE_PLUGIN_ROOT}/decision-making/problem-framing.md` |
| Personas vs. JTBD vs. Kano | `${CLAUDE_PLUGIN_ROOT}/decision-making/research-methods.md` |
| How to run a customer-discovery interview that actually produces signal | `${CLAUDE_PLUGIN_ROOT}/decision-making/customer-interviews.md` |
| What's the underlying value-hypothesis bet, and how do I test it? | `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` |
| Is this product actually defensible, and against which competitor move? | `${CLAUDE_PLUGIN_ROOT}/decision-making/competitive-moat.md` |
| How do I design a product that acts on the user's behalf (agent attribution, transparency, approval gates, autonomy)? | `${CLAUDE_PLUGIN_ROOT}/decision-making/agentic-product-design.md` |
...
| One-way vs. two-way doors, pre-mortem | `${CLAUDE_PLUGIN_ROOT}/decision-making/risk-and-reversibility.md` |
| Backlog prioritization | `${CLAUDE_PLUGIN_ROOT}/decision-making/prioritization.md` |
| What to measure | `${CLAUDE_PLUGIN_ROOT}/decision-making/metrics.md` |
| PM/engineering tension | `${CLAUDE_PLUGIN_ROOT}/cross-functional/engineering-partnership.md` |
| Project failure / kill / postmortem | `${CLAUDE_PLUGIN_ROOT}/cross-functional/failure-management.md` |
| Bundle effects, exec credibility, B2B distribution | `${CLAUDE_PLUGIN_ROOT}/cross-functional/stakeholder-alignment.md` |
