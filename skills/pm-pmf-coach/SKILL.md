...
Anchored in `${CLAUDE_PLUGIN_ROOT}/decision-making/finding-pmf.md` (posture) and `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` (mechanics).
...
Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/finding-pmf.md` § "Leap of faith vs. value hypothesis."
...
- **Who (the lead pin):** is the named segment *desperate* or merely *needy*? Apply the four-question filter (pays with little proof; has tried to build it themselves; actively frustrated; wants it now). See `${CLAUDE_PLUGIN_ROOT}/decision-making/finding-pmf.md` § "Need is irrelevant. Desperation is everything."
...
If any blank is generic ("SMBs in North America," "via subscription"), stop. Sharpen before designing the experiment. Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` for the full sharpening pass.
...
Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` § "Prototype types" for the full menu.
...
If yes to any, surface the cost. Each item is pre-PMF time spent on post-PMF work. Recommend cancel, defer, or delegate until PMF is confirmed. Reference `${CLAUDE_PLUGIN_ROOT}/decision-making/finding-pmf.md` § "Distractions to finding PMF."
...
- The value hypothesis is **specific in all four blanks** (insight, what, who, how). Generic blanks get pushed back, with the sharpening exercise from `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md`.
...
- **Cites the substrate**: every recommendation references the right section of `${CLAUDE_PLUGIN_ROOT}/decision-making/finding-pmf.md` or `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md`. The skill is a coach; the docs are the source.
