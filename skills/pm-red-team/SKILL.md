...
- **If the first pass applied the five-criterion rubric** (`${CLAUDE_PLUGIN_ROOT}/rubrics/pm-evaluation-rubric.md`): run the **stakeholder lens** — whose perspective is missing? Procurement, support, sales reps, finance, regulatory, downstream teams. (See `${CLAUDE_PLUGIN_ROOT}/cross-functional/stakeholder-alignment.md` § "Bundle thinking.")
- **If the first pass focused on user problem and value**: run the **business viability lens** — unit economics, GTM motion, distribution, sales-rep economics, pricing power. (See `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md` and `${CLAUDE_PLUGIN_ROOT}/decision-making/competitive-moat.md`.)
- **If the first pass focused on strategy and metrics**: run the **execution lens** — what does the team's history say about its ability to deliver? Engineering morale, dependencies, "this time is different" credibility. (See `${CLAUDE_PLUGIN_ROOT}/cross-functional/engineering-partnership.md` and `${CLAUDE_PLUGIN_ROOT}/cross-functional/failure-management.md`.)
...
- **If the first pass focused on metrics**: run the **gaming lens** — how would a team optimize this metric while making the underlying outcome worse? (See `${CLAUDE_PLUGIN_ROOT}/decision-making/metrics.md` § "When the metrics are lying.")
...
- **Cites the repo.** When pointing at a missed lens, name the file (`${CLAUDE_PLUGIN_ROOT}/cross-functional/stakeholder-alignment.md`, `${CLAUDE_PLUGIN_ROOT}/decision-making/value-hypothesis.md`, etc.) so the user can deepen the read.
